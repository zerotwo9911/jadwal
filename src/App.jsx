...
import { Book, Save, Calendar, User, Sparkles, Lock, LogIn, AlertCircle } from 'lucide-react';
...
export default function App() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-indigo-50 font-sans text-indigo-600 font-bold">
      Sinkronisasi Jadwal...
    </div>
  );
      ...
      <nav className="fixed top-0 inset-x-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 py-3">
        <div className="max-w-lg mx-auto flex justify-between items-center">
          ...
          <button 
            ...
            className="text-xs font-bold px-4 py-2 bg-indigo-600 text-white rounded-full"
          >
      ...
      <main className="max-w-lg mx-auto px-4 pt-20">
        {view === 'home' ? (
          <div className="space-y-6">
            <DayCard title="Hari Ini" dayName={days[new Date().getDay()]} data={scheduleData[days[new Date().getDay()]]} isToday={true} />
            <DayCard title="Besok" dayName={days[(new Date().getDay() + 1) % 7]} data={scheduleData[days[(new Date().getDay() + 1) % 7]]} isToday={false} />
          </div>
        ) : (
          isAuthenticated ? 
          <AdminPanel data={scheduleData} onSave={fetchData} /> : 
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap gap-2">
          {data.subjects?.map((s, i) => (
            <span key={i} className={`px-3 py-1 rounded-full text-sm font-medium ${isToday ? 'bg-white/20' : 'bg-slate-100'}`}>
              {s}
            </span>
          ))}
        </div>
        <div className="pt-4 border-t border-white/10 flex justify-between text-sm">
          <div>
            <p className="opacity-60 text-[10px] font-bold uppercase tracking-wider">Piket</p>
            {data.picket?.join(', ') || '-'}
          </div>
          <div className="text-right">
            <p className="opacity-60 text-[10px] font-bold uppercase tracking-wider">Tugas / PR</p>
            {data.homework || '-'}
          </div>
        </div>
      </div>
...
function AdminPanel({ data, onSave }) {
  ...
  const handleUpdate = async () => {
    setSaving(true);
    // Menggunakan upsert agar menimpa data berdasarkan kolom 'hari'
    const { error } = await supabase
      .from('Jadwal_kelas')
      .upsert({ hari: day, data: form }, { onConflict: 'hari' });
    
    if (!error) {
      alert('Berhasil Update ke Cloud! Semua teman akan melihat perubahan.');
      onSave();
    } else {
  ...
  return (
    <div className="bg-white p-6 rounded-3xl shadow-xl space-y-4 border border-slate-100">
      <label className="text-xs font-bold text-slate-400 uppercase">Pilih Hari</label>
      <select 
        value={day} 
        onChange={(e) => setDay(e.target.value)} 
        className="w-full p-4 bg-slate-50 rounded-xl font-bold border-none outline-none ring-2 ring-indigo-50"
      >
      ...
      <input 
        ...
        placeholder="Pelajaran (pisahkan dengan koma)" 
        value={form.subjects.join(', ')} 
        onChange={e => setForm({...form, subjects: e.target.value.split(',')})} 
      />
      
      <input 
        className="w-full p-4 bg-slate-50 rounded-xl outline-none" 
        placeholder="Petugas Piket (pisahkan dengan koma)" 
        value={form.picket.join(', ')} 
        onChange={e => setForm({...form, picket: e.target.value.split(',')})} 
      />
      
      <input 
        className="w-full p-4 bg-slate-50 rounded-xl outline-none" 
        placeholder="Info Tugas / PR" 
        value={form.homework} 
      ...
      <button 
        ...
        disabled={saving} 
        className={`w-full py-4 bg-indigo-600 text-white rounded-xl font-bold shadow-lg transition-all active:scale-95 ${saving ? 'opacity-50' : ''}`}
      >
...
function AdminLogin({ onLogin }) {
  ...
  return (
    <div className="bg-white p-8 rounded-3xl shadow-xl text-center space-y-4 border border-slate-100">
      <div className="bg-slate-100 w-16 h-16 rounded-2xl flex items-center justify-center mx-auto text-slate-600 mb-2">
        <Lock size={32} />
      </div>
      <h2 className="font-bold text-xl text-slate-800">Login Admin</h2>
      <p className="text-sm text-slate-500">Masukkan password untuk mengedit jadwal publik.</p>
      <input 
        type="password" 
        value={p} 
        onChange={e => setP(e.target.value)} 
        className="w-full p-4 bg-slate-50 rounded-xl text-center font-bold tracking-widest outline-none border-2 border-slate-50 focus:border-indigo-100" 
        placeholder="••••••••" 
      />
      <button 
        onClick={() => p === 'TKJ2025' ? onLogin() : alert('Password Salah!')} 
