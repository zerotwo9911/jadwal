...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-50 text-slate-800 pb-24 font-sans">
        ...
        <div className="max-w-lg mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 uppercase tracking-widest">Cloud Sync</p>
            </div>
          </div>
        ...
        {view === 'home' ? (
          <>
            ...
            <div className="flex items-center gap-4 py-2">
              ...
              <span className="text-xs text-slate-400 font-medium uppercase tracking-widest">Selanjutnya</span>
              <div className="h-px bg-slate-200 flex-1"></div>
            </div>
...
function DayCard({ title, dayName, data, isToday }) {
  if (!data) return null;
  return (
    <div className={`relative rounded-3xl p-6 transition-all duration-500 ${isToday ? "bg-gradient-to-br from-indigo-600 to-purple-700 text-white shadow-xl shadow-indigo-200" : "bg-white/80 backdrop-blur-xl text-slate-600 border border-white shadow-lg"}`}>
      <div className="flex justify-between items-start mb-6">
        <div>
          <div className={`text-xs font-bold uppercase mb-1 ${isToday ? 'text-indigo-200' : 'text-indigo-500'}`}>{title}</div>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {data.subjects.map((sub, idx) => (
            <span key={idx} className={`px-4 py-1.5 text-sm rounded-full font-medium ${isToday ? "bg-white/20 text-white border border-white/20" : "bg-indigo-50 text-indigo-600 border border-indigo-100"}`}>{sub}</span>
          ))}
        </div>
      </div>
      <div className={`grid grid-cols-2 gap-4 pt-4 border-t ${isToday ? 'border-white/20' : 'border-slate-100'}`}>
        <div>
          <div className={`flex items-center gap-1.5 text-xs font-bold uppercase mb-1 ${isToday ? 'text-indigo-200' : 'text-slate-400'}`}><User size={14}/> Piket</div>
          <div className="text-sm font-medium">{data.picket.join(', ') || '-'}</div>
        </div>
        <div className={`rounded-xl p-3 ${isToday ? 'bg-white/10' : 'bg-red-50'}`}>
          <div className={`flex items-center gap-1.5 text-xs font-bold uppercase mb-1 ${isToday ? 'text-red-200' : 'text-red-500'}`}><AlertCircle size={14}/> Tugas</div>
...
function AdminPanel({ data, setData, days }) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-6 shadow-xl border border-white space-y-6">
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {days.map(day => (
          <button key={day} onClick={() => setSelectedDay(day)} className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${selectedDay === day ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-100 text-slate-500'}`}>{day}</button>
        ))}
      </div>
      <div className="space-y-4">
        <div>
          <label className="text-xs font-bold text-slate-500 uppercase">Mata Pelajaran (koma)</label>
          <textarea className="w-full p-4 mt-1 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-indigo-500 font-medium" rows="3" value={formData.subjects.join(', ')} onChange={(e) => setFormData({...formData, subjects: e.target.value.split(',').map(s=>s.trim())})} />
        </div>
        <div>
          <label className="text-xs font-bold text-slate-500 uppercase">Piket (koma)</label>
          <input className="w-full p-4 mt-1 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-indigo-500 font-medium" type="text" value={formData.picket.join(', ')} onChange={(e) => setFormData({...formData, picket: e.target.value.split(',').map(s=>s.trim())})} />
        </div>
        <div>
          <label className="text-xs font-bold text-slate-500 uppercase">Tugas / PR</label>
          <input className="w-full p-4 mt-1 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-indigo-500 font-medium" type="text" value={formData.homework} onChange={(e) => setFormData({...formData, homework: e.target.value})} />
        </div>
      </div>
  ...
  return (
    <div className="bg-white/80 p-8 rounded-3xl shadow-xl text-center space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <input type="password" value={pw} onChange={(e) => setPw(e.target.value)} placeholder="Password..." className="w-full p-4 bg-slate-50 rounded-xl text-center font-bold tracking-widest border-none" autoFocus />
        <button type="submit" className="w-full bg-slate-900 text-white font-bold py-4 rounded-xl flex justify-center items-center gap-2 hover:bg-slate-800 transition-all shadow-lg shadow-slate-200"><LogIn size={18} /> Masuk</button>
      </form>
    </div>
  );
      }
