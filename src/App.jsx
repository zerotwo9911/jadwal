...
export default function App() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-indigo-50 font-bold text-indigo-600">Memuat Jadwal...</div>;

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-50 text-slate-800 pb-24 font-sans">
      <nav className="fixed top-0 inset-x-0 z-30 bg-white/70 backdrop-blur-xl border-b border-white/20 shadow-sm px-4 py-3">
        <div className="max-w-lg mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2">
            <div className="bg-indigo-600 p-2 rounded-xl text-white shadow-lg"><Calendar size={20} /></div>
            <div><h1 className="text-lg font-bold">Kelas Kita</h1><p className="text-[10px] text-slate-500 uppercase tracking-widest">Cloud Sync</p></div>
          </div>
  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-6 shadow-xl border border-white space-y-6">
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {days.map(d => (
          <button key={d} onClick={() => setDay(d)} className={`px-4 py-2 rounded-full text-sm font-bold ${day === d ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-100 text-slate-500'}`}>{d}</button>
        ))}
      </div>
      <div className="space-y-4">
        <div><label className="text-xs font-bold text-slate-500 uppercase">Pelajaran (pisahkan koma)</label>
        <textarea className="w-full p-4 mt-1 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-indigo-500 font-medium" rows="3" value={form.subjects.join(', ')} onChange={(e) => setForm({...form, subjects: e.target.value.split(',').map(s=>s.trim())})} /></div>
        <div><label className="text-xs font-bold text-slate-500 uppercase">Piket (pisahkan koma)</label>
        <input className="w-full p-4 mt-1 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-indigo-500 font-medium" type="text" value={form.picket.join(', ')} onChange={(e) => setForm({...form, picket: e.target.value.split(',').map(s=>s.trim())})} /></div>
        <div><label className="text-xs font-bold text-slate-500 uppercase">Tugas / PR</label>
        <input className="w-full p-4 mt-1 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-indigo-500 font-medium" type="text" value={form.homework} onChange={(e) => setForm({...form, homework: e.target.value})} /></div>
      </div>
...
function AdminLogin({ onLogin }) {
  ...
  return (
    <div className="bg-white/80 p-8 rounded-3xl shadow-xl text-center space-y-4 border border-slate-100">
